...
export default function WendGame() {
  ...
  return <DailyGameShell game="wend" title="Trace the hidden words." kicker="Wend / Word paths" description="Nối các chữ cái kề nhau để tìm toàn bộ từ ẩn. Mỗi ô chỉ được dùng một lần — hãy đi chậm, nhưng đừng mất nhịp." dayKey={dayKey} started={started} complete={state.complete} timeMs={elapsed} hintsUsed={state.hintsUsed} scores={scores} onStart={begin} onHint={() => { begin(); setState((current) => applyWendHint(current)); }} onReset={reset} onUndo={() => setSelection((current) => current.slice(0, -1))} undoDisabled={!selection.length} onClear={() => setSelection([])} clearDisabled={!selection.length}>
    <div className="wend-instructions"><span>{started ? 'Tap letters in sequence, then submit the word.' : 'A new word board is ready.'}</span><span>{state.found.length} / {puzzle.words.length} words found</span><span>{state.mistakes ? `${state.mistakes} wrong trace${state.mistakes === 1 ? '' : 's'}` : 'No wrong traces yet.'}</span></div>
