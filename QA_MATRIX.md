...
## Flappy Bird integration QA

The requested [noanonoa/flappy-bird repository](https://github.com/noanonoa/flappy-bird) was cloned and audited. GitHub reports no declared repository license and the upstream README attributes its image/audio assets to third-party sources, so no upstream source assets were copied. Signal Room instead ships a clean-room canvas adaptation documented in `FLAPPY_BIRD_INTEGRATION.md`.

Production browser QA confirmed a seventh accessible tab (`game-tab-flappy`), the Flappy Bird panel, responsive canvas markup, local best-score copy, and touch/keyboard instructions. The game transitions from ready to gameover when started without a flap, then a controlled Space dispatch after reset changes the canvas stage to `is-playing` with `State ON`. The canvas is focusable and exposes `role="img"` plus a descriptive instruction paragraph. The implementation uses pointer-down, Space, ArrowUp and W input, `requestAnimationFrame`, DPR-aware resize, touch-action suppression, cleanup on tab unmount and safe localStorage fallback.

The Play hero copy was updated from six to seven games and from four to five replay games after browser QA found the stale count. The new Flappy Bird engine helper tests cover deterministic gap generation, ceiling/ground bounds and pipe collision; the total suite is now 28/28 passing.
