...
import { capabilities, posts, processSteps, projects } from '@/lib/content';
...
export default function Home() {
  ...
  return <main className="new-site-shell">
    ...
    <section id="services" data-home-section className="new-section new-capabilities"><div className="new-section-intro"><div className="new-index">03 <span>What I can help with</span></div><div><h2>Useful from the<br /><em>first question.</em></h2><p>Không chỉ làm cho sản phẩm trông tốt hơn. Mình giúp biến một bài toán còn mơ hồ thành hệ thống có thể hiểu, thử và phát triển.</p></div></div><div className="capability-grid">{capabilities.map((capability) => <article className="capability-card" key={capability.number}><div className="capability-card-top"><span>{capability.number}</span><span>Signal / {capability.number}</span></div><h3>{capability.title}</h3><p>{capability.description}</p><div className="capability-tags">{capability.bullets.map((bullet) => <span key={bullet}>{bullet}</span>)}</div></article>)}</div></section>

    <section id="process" data-home-section className="new-section new-process"><div className="new-section-intro"><div className="new-index">04 <span>How I work</span></div><div><h2>Less theatre,<br /><em>more signal.</em></h2><p>Một quy trình tốt không làm mọi thứ chậm lại. Nó giúp cả team biết điều gì cần được hiểu, quyết định và ship tiếp theo.</p></div></div><div className="process-list">{processSteps.map((step) => <div className="process-row" key={step.number}><span>{step.number}</span><div><h3>{step.title}</h3><p>{step.description}</p></div><ArrowDownRight size={18} /></div>)}</div></section>
    ...
    <footer id="contact" className="new-footer"><div className="new-footer-top"><div className="new-index">07 <span>Get in touch</span></div><div><h2>Have a good<br /><em>problem?</em></h2><p className="new-footer-intro">Đang mở cho những dự án nhỏ có câu hỏi lớn — product direction, frontend systems hoặc một prototype cần được đưa ra ánh sáng.</p><a className="new-footer-email" href="mailto:hello@phanvinh.id.vn">hello@phanvinh.id.vn <ArrowUpRight size={24} /></a></div></div><div className="new-footer-availability"><span><i /> Currently open to thoughtful collaborations</span><span>Remote / Vietnam time</span></div><div className="new-footer-bottom"><span>© 2026 Phan Vinh / Built with intention</span><div className="new-socials"><a href="https://github.com/PhanVinh2k6" target="_blank" rel="noreferrer" aria-label="GitHub"><Github size={17} /></a><a href="https://www.linkedin.com" target="_blank" rel="noreferrer" aria-label="LinkedIn"><Linkedin size={17} /></a><a href="mailto:hello@phanvinh.id.vn" aria-label="Email"><Mail size={17} /></a></div><a href="#top">Back to top ↑</a></div></footer>
  </main>;
}
