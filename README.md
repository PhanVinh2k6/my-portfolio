...
The website logo now uses the supplied Phan Vinh portrait as a transparent avatar in `public/phan-vinh-avatar.webp`. Flappy Bird is documented in `FLAPPY_BIRD_INTEGRATION.md`; the upstream repository has no declared license and attributes visual/audio materials to third-party sources, so Signal Room uses a clean-room canvas adaptation of the gameplay idea rather than copying unverified source assets. The arcade boot/play loader uses the same portrait instead of the former `PV` text mark, with a restrained scanline, ring glow and float effect that is disabled under reduced motion. `app/favicon.ico`, `public/icon-192.png`, `public/icon-512.png` and `public/apple-touch-icon.png` are generated from the same alpha-preserving source, and `app/manifest.ts` exposes all PWA icon sizes. The service worker cache is bumped to `signal-room-v3` so returning offline clients refresh the branding assets.
